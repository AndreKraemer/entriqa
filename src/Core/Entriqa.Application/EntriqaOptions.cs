...
public sealed class EntriqaOptions
{
    ...
    public string DefaultTimeZone { get; set; } = "Europe/Berlin";          // #7: appointment labels when the visitor sent no (known) zone

    /// <summary>
    /// The ceiling one inbox search honours, never below one. A configured zero is the misconfiguration
    /// that costs nothing to make and everything to diagnose: every search would scan nothing, find
    /// nothing and report that it searched no submissions - indistinguishable from a term nobody matches.
    /// </summary>
    public int EffectiveSearchScanMax => Math.Max(1, SearchScanMax);
